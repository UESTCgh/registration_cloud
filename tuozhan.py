...
#bunny
voxel_size=0.03#采样格子大小max 初始0.05
threshold=0.03# RMSE残差网值，小于该残差闯值，迭代终止0.02 0.002
max_iterations=64 # 配准的最大迭代次数 64
max_tuples=1000#最多同名点个数   1000
distance_threshold=1.5*voxel_size# 同名点之间距离阙值

#获取示例数据

src1 = o3d.io.read_point_cloud("lab2_sfm.ply")
dst = o3d.io.read_point_cloud("lab2_kinect.ply")
...
# 显示配准之前的点云
o3d.visualization.draw_geometries([src,dst], window_name="未配准",
...
# 显示配准之前的点云
o3d.visualization.draw_geometries([src1,dst], window_name="粗配准",
                                width=1024, height=768,
                                left=50, top=50,
                               mesh_show_back_face=False)
...
o3d.visualization.draw_geometries([src2,dst], window_name="精配准",
                                width=1024, height=768,
                                left=50, top=50,
                               mesh_show_back_face=False)
